...
*/
static int Parse_bstr_to_wordset_of_sol(int m, std::string &str, Word_set &word_set)
{
    //FuncBegin();
...
static int Parse_label_to_word_sets_of_sol(int i, std::string &label, std::vector<Word_set> &word_sets,ofstream& ofile)
{
    FuncBegin();
    ...
    for (auto &t : label_set)
    {
        ...
        Parse_bstr_to_wordset_of_sol(m, t, ws);
...
int export_automata_to_solidity(Monitor &monitor, spot::twa_graph_ptr &aut, const spot::bdd_dict_ptr &dict)
{
    ...
    for (num_state; num_state < aut->num_states(); ++num_state) //对每个状态赋值
    {
        ...
        for (auto &t : aut->out(num_state))
        {
            ...
            if (Parse_label_to_word_sets_of_sol(int i, monitor_label.label, monitor_label.word_sets,ofile) != SUCCESS)
            {
                AMReturn(ERROR);
            }
