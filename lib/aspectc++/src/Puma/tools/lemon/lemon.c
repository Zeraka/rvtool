...
struct lemon {
  ...
  char *outdirname;        /* Name of the output directory */
...
int main(int argc, char **argv)
{
  ...
  if( OptNArgs()!=2 ){
    fprintf(stderr,"usage: lemon [options] grammar outputdir\n");
    exit(1);
  }
...
PRIVATE char *file_makename(struct lemon *lemp, const char *suffix)
{
  ...
  name = (char*)malloc( lemonStrlen(lemp->outdirname) + lemonStrlen(lemp->filename) + lemonStrlen(suffix) + 5 );
  if( name==0 ){
    fprintf(stderr,"Can't allocate space for a filename.\n");
    exit(1);
  }
  lemon_strcpy(name,lemp->outdirname);
  if( name[lemonStrlen(name)-1] != '/' )
    lemon_strcat(name,"/");
  cp = strrchr(lemp->filename,'/');
  if( cp )
    lemon_strcat(name,cp+1);
  else
