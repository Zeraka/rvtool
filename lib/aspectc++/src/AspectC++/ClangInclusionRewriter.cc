...
void InclusionRewriter::CommentOutDirective(Lexer &DirectiveLex,
  ...
  // The eod-token can contain a multi-line comment. Therefore we end
  // our "commented-out" comment after the token, that is in front of the
  // eod-token. Afterwards we write the eod-token.
  Token DirectiveToken;
  Token prev_directive_token;
  int token_count = 0;
  do {
    prev_directive_token = DirectiveToken;
    DirectiveLex.LexFromRawLexer(DirectiveToken);
    ++token_count;
  } while (!DirectiveToken.is(tok::eod) && DirectiveToken.isNot(tok::eof));
//  OS << "#if 0 /* expanded by -frewrite-includes */" << EOL;
  if(token_count <= 1) {
    // If DirectiveToken is still the first token, then we have no previous token.
    // Set prev_directive_token to the first token:
    prev_directive_token = DirectiveToken;
  }
  OS << " /* commented-out by ac++ include expander: ";
  OutputContentUpTo(FromFile, NextToWrite,
    SM.getFileOffset(prev_directive_token.getLocation()) + prev_directive_token.getLength(),
    EOL, Line);
  OS << " */ ";
  OutputContentUpTo(FromFile, NextToWrite,
...
bool InclusionRewriter::HandleHasInclude(
  ...
  const FileEntry *File = PP.getHeaderSearchInfo().LookupFile(
      Filename, isAngled, 0, CurDir, PP.getSourceManager().getFileEntryForID(FileId), 0, 0, 0, false);
#else // C++ 11 interface
    ...
    if (RawToken.is(tok::hash) && RawToken.isAtStartOfLine()) {
      ...
      if (RawToken.getIdentifierInfo() != NULL) {
        switch (RawToken.getIdentifierInfo()->getPPKeywordID()) {
          ...
          case tok::pp_import: {
            const FileChange *Change = FindFileChangeLocation(
                HashToken.getLocation());
            if (aspect && Change && !aspect->should_expand_file(FileId,
                                                                Change->Id))
              break;
            ...
            if (Change) {
              ...
              // new for AspectC++
              PresumedLoc PLoc = SM.getPresumedLoc(HashToken.getLocation());
              WriteLineInfo(PLoc.getFilename (), PLoc.getLine() + 1, FileType, EOL, LineInfoExtra);
              // end new
            }
            // fix up lineinfo (since commented out directive changed line
            // numbers) for inclusions that were skipped due to header guards
            // not needed for AspectC++
